...
balance_summary |> 
 ...
 tab_header(
  ...
  # Add a Spanner to group the data columns - underline spanning column names 
tab_spanner(
...
tbl_2way <- lobsters |> 
  ...
  # Pivot to create a column for each region
  pivot_wider(
    names_from = region, 
...
```{r}
# Because we are coming from dataframe use gt()
tbl_2way |> 
  gt(rowname_col = "year") |> 
  tab_header(
...
tbl_3way <- lobsters |> 
  ...
  summarise(total_cpue = sum(cpue, na.rm = TRUE), 
            .groups = "drop") |> 
  # Pivot to creat column for each treatment/control group within each region 
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "buffalo"); beep(9)
...
Air quality issues in the global south due to informal burning of household waste. 

2. How were the treatment and control groups formed?

44 neighborhoods were chosen, half participated in pair-wise competition to minimize informal waste-burning over 8 months. 

3. Discuss the matched-pairs design of the study. How were neighborhoods paired? Create a diagram if helpful!

All zones received education about dangers of open-air. Pair-wise competition to minimize informal waste-burning over 8 months. 

4. Was the social competition strategy effective?

Yes, there was a 24% reduction in informal waste burning when involved in competition. "Treated" zones made more of an effort to transition to formal waste disposal. 

5. What was the primary metric for assessing the impact of the social competition?

Transect based audits: recording the number of waste piles. 
