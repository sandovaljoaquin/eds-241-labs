...
```
Intercept (77.91): Without any treatment, we expect the number of trash piles should be ~ 78 on average. 

Treat(-24.77): The treatment reduced the average number of trash piles by ~ 25 piles. 
...
```
Intercept: On average, the model estimates our control group to have 78 piles. 
Coefficient: The model estimates that the treatment reduced waste burning by 32%. 
...
```
Intercept (74.44049): On average, the model estimates our control group to have 74 piles
treat (34.29532): On average, the model estimates that the treatment reduced waste burning by 34%.
...
m2_change = (exp(-.38) - 1) * 100 # Change in POIS: -31.6
m3_change = (exp(-.42) - 1) * 100 # Change in log OLS: -34.3
```
...
:::
The treatment group already has a lower mean pre count average of trash piles than the control group. 
...
```
Intercept (1.599994): Model predicts 1.6 waste piles_____
treat (-23.66205): On average, model predicts a 24% reduction in waste piles in treated group when controlling for pre_count_avg waste piles. 
log(pre_count_avg) (133.9647): On average, a one unit change in precount average. 
