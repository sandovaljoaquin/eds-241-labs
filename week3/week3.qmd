...
weekly_la_phx <- did_data_az_ca |> 
  ...
  group_by(week, city) |> 
  # Every week, mean no2 sum for LA and Pheonix 
  summarise(mean_no2 = mean(daily_max_1_hour_no2_concentration, na.rm = TRUE), 
            .groups = "drop")

# Create a line plot of Pheonix and LA weekly NO2 levels 

ggplot(weekly_la_phx, aes(x = week, 
                          y = mean_no2, 
                          color = city)) + 
  geom_line(size = 1.2) + 
  geom_point(size = 2.5) + 
  
  # Add CA lockdown start date line 
...
annotate("label", x = as.Date("2020-03-20"), 
         ...
         label = "CA Lockdown\n Starts", 
         size = 3) + 
  labs(title = "Weekly NO2 Trends in 2020: Los Angeles vs Pheonix", 
       x = "Date", 
...
No. Prior to the lockdown, both areas exhibited different weekly Max NO2 rates. Parallel assumption is violated. 
...
This is a better comparison. There is still a discrepancy between the two but the lines have similar trends and periods of being somewhat parellel to eachother. 
...
treatment_effect <- (y_treatment_post - y_treatment_pre) - (y_control_post - y_control_pre)

print(paste("The treatment effect is", round(treatment_effect, 4)))
```
...
What does this treatment effect of -3.5162 mean? Talk with a partner! 
:::

The treatment effect is -3.5162: average daily [NO2] is 3.5162 units less in New Mexico than Texas. 
...
1. Before the lockdown, we see week-to-week fluctuations in NO2 in New Mexico and Texas. Do you think that Texas makes a compelling counterfactual which satisfies the main causal  identification assumption for DiD?

**Your answer here.**

Yes, based on the [NO2] prior to the lockdown, trends in both groups were pretty similar: assumption of parallel trends prior to treatment was satisfied. Data was pretty volatile, makes it harder for model to pick up trends. 
...
treated:is_post represents the treatment effect. Its p value was not significant (.2210). We can get this difference in control and treatment 22% of time by the chance.  
...
The counterfactual is Texas. Both counties are next to eachother: similar geographic/climate features but had different policies. Massive dust storm would not ruin DiD estimate: both counties would have been effected. 
...
It would remain the same. Both behaviors would lower [NO2] but the differenc between control and treatment levels would be the same.
