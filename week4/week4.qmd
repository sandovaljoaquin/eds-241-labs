...
data_clean <- read_csv(here("week4", "data", "Ahmadia_2015_DataClean.csv"), show_col_types = FALSE)
```
...
*Response:* Reef Exposure: Control Exposed	35 (66%)	MPA Exposed: 92 (85%). 

Pollution risk: Control Low	33 (62%), MPA 84 (78%)

Distance to mangroves: Control 9,679 (14,229), MPA 4,618 (5,165)
...
*Response:* Distance to mangroves: shorter distance to mangroves might increase fish biomass due to ease of accessing juvenile fish habitat. 
...
*Response:* Average treatment effect on treated: Fish biomass within MPAs, if the MPA were not MPAs. 
...
match_model <- matchit(
    # Similar to a linear model, using covariates we want to match
    treated_mpa ~ dist_to_deep_water + 
        ssta_freq + 
        reef_exposure + 
        reef_slope + 
        reef_type + 
    ...
    #Resuse control sites since we didn't have enough contorl
    replace = TRUE
)
```
...
*Response:* 108 treated units were matched, 37 control sites were included in the matched data. 

**Q6.** Find the covariate with the largest standardized mean difference (SMD) *after matching*. Which covariate is it?

*Response:* The covariate with the largest SMD after matching is dist_to_deep_water (0.3097). 
...
*Response:*  Ecological: On average, MPA sites are expected to have 79.50 kgs/hectare more herbivorous fish biomass than matched control sites. 

Fisheries: On average, MPA sites are expected to have 107.78 kgs/hectare more key fishery family biomass than matched control sites.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* We could have some unobserved covariates that may be affecting the causal interpretation of coefficients after matching. 
...
```{r}
data_fe <- read_csv(here("week4", "data", "Dudney2021_study_data.csv"), show_col_types = FALSE) |> 
...
*Response:* 
no-FE model vpd:0.126
FE model vpd: 0.227

The estimate doubled. This suggests that plot was a confounding variable; some bias from our plot variable was influencing the model estimate of vpd. 
...
*Response:* Including year in fixed effects would account for temporal variation in climate that may effect estimates.   
...
*Response:* Plots may have different micro-environments (e.g soil nutrients, proximity to streams).  
